...
class ResponseValidator {
    ...
    validate(response, criteria) {
        ...
        const actualText = types_1.ResponseParser.extractText(response);
     ...
     */
    validatePattern(actual, criteria) {
     ...
     */
    validateSemantic(actual, criteria) {
        const expected = String(criteria.expected);
     ...
     */
    validateCustom(response, criteria) {
        if (typeof criteria.expected === 'function') {
            return criteria.expected(response);
        }
        return {
            passed: false,
            actual: types_1.ResponseParser.extractText(response),
     ...
     */
    validateExactMatch(response, expected) {
     ...
     */
    validatePatternMatch(response, pattern) {
     ...
     */
    validateSemanticSimilarity(response, expected, threshold) {
        // Normalize strings
     ...
     */
    calculateWordOverlapSimilarity(str1, str2) {
     ...
     */
    validateMultiple(responses, criteriaList) {
        const results = [];
     ...
     */
    validateAll(response, criteriaList) {
     ...
     */
    validateAny(response, criteriaList) {
        const results = criteriaList.map(criteria => this.validate(response, criteria));
     ...
     */
    static getFailureDetails(result) {
        return {
